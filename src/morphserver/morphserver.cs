...
namespace onesharp
{
    class morphserver : Onesharp
    {
        ...
        bool ВыполнитьЗадачу(Структура _структЗадача)
        {
            ...
            var структЗадача = _структЗадача.с;
            ...
            структЗадача.Запрос.с.Параметры.Свойство("Действие", out Команда);
            ...
            if ((string)Команда == "ФормыСлов")
            {
                ...
                while (ТекущаяУниверсальнаяДатаВМиллисекундах() - НачалоЦикла < 50)
                {
                    ...
                    var мСлова = new Массив(Стр.Разделить(структЗадача.Запрос.с.Параметры.с.Слова, Символы.ПС));
                    ...
                    { // форма слова не найдена
                        ...
                        { // е заменить на ё
                            ...
                            for (н = 1; н <= дслф; н++) {
                                if (Сред(слф, н, 1) == "Е")
                                {
                                    структЗадача.Запрос.с.Параметры.с.Слова = структЗадача.Запрос.с.Параметры.с.Слова + Символы.ПС + Лев(слф, н - 1) + "Ё" + Сред(слф, н + 1);
                                }
                            }
                        }
                    }
            ...
            else if ((string)Команда == "Грамматики")
            {
                ...
                string сл = структЗадача.Запрос.с.Параметры.с.Слова;
            ...
            else if ((string)Команда == "Элемент")
            {
                ...
                var поз = (int)Число(структЗадача.Запрос.с.Параметры.с.Позиция);
                var Раскрыть = Истина;
                //структЗадача.Запрос.Свойство("Раскрыть", Раскрыть);

                string ИмяДанных = структЗадача.Запрос.с.Параметры.с.База;
