...
namespace onesharp
{
    class morphserver : Onesharp
    {
        ...
        bool ВыполнитьЗадачу(Структура _структЗадача)
        {

            object Команда = "";

            var структЗадача = _структЗадача[d];
            ...
            структЗадача.Запрос[d].Параметры.Свойство("Действие", out Команда);
            ...
            if ((string)Команда == "ФормыСлов")
            {
                ...
                while (ТекущаяУниверсальнаяДатаВМиллисекундах() - НачалоЦикла < 50)
                {
                    ...
                    var мСлова = new Массив(Стр.Разделить(структЗадача.Запрос[d].Параметры[d].Слова, Символы.ПС));
                    ...
                    { // форма слова не найдена
                        ...
                        { // е заменить на ё
                            ...
                            for (н = 1; н <= дслф; н++) {
                                if (Сред(слф, н, 1) == "Е")
                                {
                                    структЗадача.Запрос[d].Параметры[d].Слова = структЗадача.Запрос[d].Параметры[d].Слова + Символы.ПС + Лев(слф, н - 1) + "Ё" + Сред(слф, н + 1);
                                }
                            }
                        }
                    }
            ...
            else if ((string)Команда == "Грамматики")
            {
                ...
                string сл = структЗадача.Запрос[d].Параметры[d].Слова;
            ...
            else if ((string)Команда == "Элемент")
            {
                ...
                var поз = (int)Число(структЗадача.Запрос[d].Параметры[d].Позиция);
                var Раскрыть = Истина;
                //структЗадача.Запрос.Свойство("Раскрыть", Раскрыть);

                string ИмяДанных = структЗадача.Запрос[d].Параметры[d].База;
                ...
                if (поз == 0)
                { // начало файла
                    м = База.ПолучитьЗначения(Элемент[d].Соседний);
                    з = ИмяДанных;
                }
                else
                {
                    м = База.ПолучитьЗначения(Элемент[d].Дочерний);
                    ...
                    { // текст
                        ...
                        if (!(Элемент[d].Значение < 16 && Раскрыть == Истина))
                        {
                            if (м.Количество() == 1)
                            { // раскрыть дерево
                                while (м.Количество() == 1)
                                {
                                    var эл = База.ПолучитьЭлемент(Элемент[d].Дочерний);
                                    if (эл.Значение < 16)
                                    {
                                        break;
                                    }
            ...
            while (!(ОстановитьСервер))
            {
                ...
                while (к > 0 && !(ТекущаяУниверсальнаяДатаВМиллисекундах() - НачалоЦикла > 50))
                {
                    ...
                    catch (Exception e)
                    {
                        ...
                        Задачи.Удалить(структЗадача[d].ИдЗадачи);
                        continue;
                    }
                    if (ЕстьРезультат == Истина)
                    {
                        try
                        {
                            ...
                            if (структЗадача[d].Запрос.Свойство("ОбратныйЗапрос", out ОбратныйЗапрос))
                            { // возвращаем результат
                                ОбратныйЗапрос.Вставить("РезультатДанные", Структура.Новый("Ответ, Результат", структЗадача[d].Ответ, структЗадача[d].Результат));
                                if (Функции.ПередатьДанные(ОбратныйЗапрос[d].Хост, (int)Число(ОбратныйЗапрос[d].Порт), ОбратныйЗапрос) == Неопределено)
                                {
                                    continue;
                                }
                                Сообщить("morphserver " + ТекущаяДата() + " time=" + (ТекущаяУниверсальнаяДатаВМиллисекундах() - структЗадача[d].ВремяНачало) + Загрузка);
                ...
                while (к > 0)
                {
                    ...
                    if (Соединение.Статус == "Данные")
                    {
                        ...
                        if (!(Запрос == Неопределено))
                        {
                            ...
                            Задачи.Вставить(структЗадача[d].ИдЗадачи, структЗадача);
                            мЗадачи.Добавить(структЗадача);
