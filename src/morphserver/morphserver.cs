        ...
        bool ВыполнитьЗадачу(ТипЗадача структЗадача)
        {
            ...
            if (Команда == "ФормыСлов")
            {
                ...
                while (ТекущаяУниверсальнаяДатаВМиллисекундах() - НачалоЦикла < 50)
                {
                    ...
                    Массив мСлова = Стр.Разделить(структЗадача.Запрос.Параметры.Слова, Символы.ПС);
            ...
            while (!(ОстановитьСервер))
            {
                ...
                while (к > 0 && !(ТекущаяУниверсальнаяДатаВМиллисекундах() - НачалоЦикла > 50))
                {
                    ...
                    if (ЕстьРезультат == Истина)
                    {
                        try
                        {
                            Структура _ОбратныйЗапрос = null;
                ...
                while (к > 0)
                {
                    ...
                    if (Соединение.Статус == "Данные")
                    {
                        ...
                        {
                            структЗадача = ТипЗадача.Новый("ИдЗадачи, Запрос, Ответ, Результат, ВремяНачало", ПолучитьИД(), Запрос, Неопределено, Новый_Структура(), ТекущаяУниверсальнаяДатаВМиллисекундах());
