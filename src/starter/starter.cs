            ...
            while (!ЗавершитьПроцесс)
            {
                ...
                while (к > 0 && !(ТекущаяУниверсальнаяДатаВМиллисекундах() - НачалоЦикла > 50))
                {
                    ...
                    else if (cmd == "startdata")
                    {
                        ...
                        {
                            структЗадача.Запущен = Истина;
                            //ЗапуститьПроцесс("useyourmind.exe dataserver.os", ПортД);
                        }
                        else
                        {
                            continue;
                    ...
                    else if (cmd == "showdata")
                    {
                        if (showdata == 0)
                        {
                            ...
                            while (!ЗапуститьПроцесс("useyourmind.exe showdata.os", НовыйПортК))
                            //while (!ЗапуститьПроцесс("showdata.exe", НовыйПортК))
                            {
                                НовыйПортК = НовыйПортК - 1;
                            }
                    ...
                    else if (cmd == "stopserver" || cmd == "restartserver")
                    {
                        if (Локальный)
                        {
                            ...
                            {
                                Функции.ПередатьДанные(элКонтроллер.Значение.Хост, элКонтроллер.Значение.Порт, Структура.Новый("cmd, taskid", "termproc", "0"));
                            }
                        ...
                        else
                        {
                            ...
                            if (ДанныеВходящие.Свойство("procid"))
                            {
                                ...
                                if (элКонтроллер != Неопределено)
                                {
                                    Функции.ПередатьДанные(элКонтроллер.Хост, элКонтроллер.Порт, Структура.Новый("cmd, taskid", "termproc", "0"));
                                }
                            }
                        }
                        continue;
