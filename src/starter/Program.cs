...
namespace starter
{
    ...
    class starter : functions
    {
        ...
        public void Main()
        {
            ...
            Перем Хост = Строка("127.0.0.1");

            Перем Порт = Число(8890);
            Перем ПортЗ = Число(8888);
            Перем ПортВ = Число(8889);
            Перем ПортД = Число(8887);
            Перем ПортМ = Число(8886);
            decimal НовыйПортК = ПортМ.Число;
            ...
            if (АргументыКоманднойСтроки.Length != 0)
            {
                ...
                if (!Локальный)
                {
                    ПортЗ = Строка(АргументыКоманднойСтроки[1]);
            ...
            while (!ЗавершитьПроцесс)
            {
                ...
                {
                    к = к - 1;
                    Перем структЗадача = мЗадачи.Получить(0);
                    мЗадачи.Удалить(0);
                    ...
                    if (cmd == "startproc")
                    {
                        if (showdata != Неопределено) {
                            if (Контроллеры.Получить(ДанныеВходящие.procid) == Неопределено)
                            {
                                var ПортК = showdata;
                                showdata = null;
                                var стрКонтроллер = Новый_Структура("procid, Хост, Порт, ПортС, ПортВ, ПортД, ПортМ, УдаленныйУзел, Локальный, ПараметрХост, cmd", new object[] { ДанныеВходящие.procid, Хост, ПортК, Порт, ПортВ, ПортД, ПортМ, "", Локальный, ПараметрХост, "init" } );
                        ...
                        else if (cmd == "startweb")
                        {
                            if (структЗадача.Запущен == Ложь)
                            {
                                структЗадача.Запущен = ЗапуститьПроцесс("webserver.os", ПортВ, ПортЗ.Строка);
                            }
                            else
                            {
                                if (ПередатьДанные(Хост, ПортВ, Новый_Структура("Хост, Порт, ПортС, ПортД, cmd", new object[] { Хост, ПортВ, Порт, ПортД, "init" } )) != Неопределено) {
                                    Приостановить(50);
                                    continue;
                                }
                            }
                        ...
                        else if (cmd == "showdata")
                        {
                            if (showdata == Неопределено)
                            {
                                if (НовыйПортК < 5555)
                                {
                                    НовыйПортК = ПортМ.Число;
                                }
                                НовыйПортК = НовыйПортК - 1;
                                while (!ЗапуститьПроцесс("showdata.os", Число(НовыйПортК)))
                                {
                                    НовыйПортК = НовыйПортК - 1;
                                }

                                showdata = Число(НовыйПортК);
                            }
                        }
                            ...
                            if (Локальный)
                            {
                                foreach (dynamic элКонтроллер in Контроллеры)
                                    {
                                        ПередатьДанные(элКонтроллер.Значение.Хост, элКонтроллер.Значение.Порт, Новый_Структура("cmd, taskid", new object[] { "termproc", "0" } ));
                                    }
