...
namespace dataserver
{
    ...
    class dataserver : functions
    {
        ...
        object ВыполнитьЗадачу(dynamic структЗадача)
        {
            ...
            var ИдПроцесса = УзелСвойство(Запрос, "ИдПроцесса");
            ...
            while (!(ОстановитьСервер))
            {
                ...
                while (к > 0 && !(ТекущаяУниверсальнаяДатаВМиллисекундах() - НачалоЦикла > 50))
                {
                    ...
                    if (!(ЕстьРезультат))
                    {
                        ...
                        bool ВыполнитьЗадачу = (структЗадача.Этап == "Выполнить" || структЗадача.Этап == "Продолжить");
                    ...
                    if (ЕстьРезультат == Истина)
                    {
                        try
                        {
                            ...
                            { // возвращаем результат
                                dynamic Контроллер = Контроллеры.Получить(структЗадача.Запрос.ИдПроцесса) as Структура;
                                if (!(Контроллер == Неопределено))
                                {
                ...
                while (к > 0)
                {
                    ...
                    if (Соединение.Статус == "Данные")
                    {
                        ...
                        if (!(Запрос == Неопределено))
                        {
                            dynamic структЗадача = Новый_Структура("ИдЗадачи, Этап, Запрос, Ответ, Результат, ВремяНачало", ПолучитьИД(), "Выполнить", Запрос, Неопределено, Неопределено, ТекущаяДата());
