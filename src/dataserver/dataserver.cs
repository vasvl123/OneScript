        ...
        object ВыполнитьЗадачу(dynamic структЗадача)
        {
            ...
            { // выбрать данные по запросу
                ...
                { // выбрать данные по запросу
                    ...
                    Перем Ключ = структЗадача.Получить("Данные");
            ...
            else if (Команда == "ПолучитьДанные")
            {
                ...
                { // прочитать файл по позиции в контейнере
                    int ТипДанных;
                    структЗадача.Вставить("Результат", Новый_Структура("Данные", Данные.ПолучитьДанные((int)Число(ПозицияДанных), out ТипДанных)));
        ...
        void ОбработатьСоединения()
        {
            ...
            while (!(ОстановитьСервер))
            {
                ...
                while (к > 0 && !(ТекущаяУниверсальнаяДатаВМиллисекундах() - НачалоЦикла > 50))
                {
                    ...
                    bool ЕстьРезультат = (!(структЗадача.Результат is null));
                    if (!(ЕстьРезультат))
                    {
                        ...
                        try
                        {
                            ...
                            { // возвращаем результат
                                ...
                                if (!(Контроллер == Неопределено))
                                {
                                    ...
                                    структЗадача.Вставить("Соединение", ПередатьДанные(Контроллер.Хост, Контроллер.Порт, ОбратныйЗапрос));
