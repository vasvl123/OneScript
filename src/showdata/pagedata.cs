...
namespace onesharp
{
    class pagedata : Onesharp
    {
        ...
        public Узел ИмяЗначение(string Имя = "", string Значение = "")
        {
        ...
        // Создать копию ветки
        public Структура КопироватьВетку(Узел Узел, pagedata Цель, Узел Старший, Узел Родитель, bool ЭтоАтрибут = false, bool ПервыйВызов = true, bool знСлужебный = true, Структура ПараметрыЗамены = null)
        {
            ...
            if (Узел.Дочерний == Неопределено)
            {
                if ((string)ИмяУзла == "Узел")
                {
                    if (!(ПараметрыЗамены == Неопределено))
                    {
                        ...
                        {
                            КопияУзел.Вставить("Дочерний", Цель.НовыйУзел(Структура.Новый("Имя, Значение, Старший, Родитель", "Строка", Параметр, Узел, Узел), знСлужебный || Служебный(Узел)));
                        }
                    }
                }
            }
        ...
        public object СоздатьСвойства(Узел Узел, string шСвойства, object _Служебный = null, bool? вКонец = null)
        {
            ...
            var к = -(1);
            var св = Узел;
            var стрСвойства = Стр.Разделить(шСвойства, Символы.ПС);
            foreach (string стр in стрСвойства)
            { // парсер
                if (!(стр == ""))
                {
                    var м = Стр.Разделить(стр, Символы.Таб);
                    var т = 0;
                    var Имя = "";
                    var Зн = "";
                    var п = 0;
        ...
        public Узел ПолучитьУзел(object Код, Узел Старший = null)
        {
            ...
            if (!(Узел == Неопределено))
            {
                ...
                if (!(Старший == null))
                {
                    ...
                    var СтаршийСоседний  = Старший.Получить("Соседний") as string;
        ...
        public Структура НовыйРодитель(Узел Дочерний, Узел СтруктураУзла, bool Служебный = false)
        {
        ...
        public Структура НовоеЗначениеУзла(Узел Узел, object Значение = null, bool Служебный = false, object _Добавить = null, bool Обновить = true)
        {
            ...
            if (!(Значение is Узел) || !((Структура)Значение).Свойство("Имя"))
            {
                Значение = ИмяЗначение(Значение.GetType().ToString(), Значение.ToString());
            }
        ...
        public pagedata(showdata обПроцесс, string Текст = "", string знБазаДанных = "", string знИмяДанных = "", string знПозицияДанных = "") : base("pagedata")
        {
            ...
            Данные = ТекстовыйДокумент.Новый();
