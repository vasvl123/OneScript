...
namespace showdata
{
    class pagedata : functions
    {
        ...
        public new ТипУзел ИмяЗначение(string Имя = "", string Значение = "")
        {
            return new ТипУзел("Имя, Значение", Имя, Значение) as ТипУзел;
        }
        ...
        object ЗначениеСвойства(ТипУзел _знУзелСвойство, string Имя = null)
        {
            var Значение = Неопределено;
            ТипУзел знУзелСвойство = null;

            if (!(Имя == Неопределено))
            {
                знУзелСвойство = (ТипУзел)УзелСвойство(_знУзелСвойство, Имя);
            }
        ...
        string ПоказатьУзел(ТипУзел Узел, string Атрибуты = "", string Дочерний = "", bool ЭтоАтрибут = false)
        {
            var Представление = "";
        ...
        // Создать копию ветки
        public ТипУзел КопироватьВетку(dynamic Узел, pagedata Цель, ТипУзел Старший, ТипУзел Родитель, bool ЭтоАтрибут = false, bool ПервыйВызов = true, bool знСлужебный = true, Структура ПараметрыЗамены = null)
        {
            ...
            ТипУзел КопияУзел = Цель.НовыйУзел(Новый_Структура("Имя, Значение, Старший, Родитель", ИмяУзла, ЗначениеУзла, Старший, Родитель), знСлужебный || Служебный(Узел));
            ...
            if (Узел.Дочерний == Неопределено)
            {
                if (ИмяУзла == "Узел")
                {
                    if (!(ПараметрыЗамены == Неопределено))
                    {
                        ...
                        {
                            Узел.Дочерний = (ТипУзел)Параметр;
                        }
        ...
        bool СравнитьИмя(ТипУзел Узел, string Тип,string Имя)
        {
        ...
        object НайтиОбъекты(ТипУзел Узел, string оПоз, string оТип, string оИмя)
        {
        ...
        ТипУзел СвойствоОбъекта(ТипУзел Узел, string ИмяСвойства)
        {
            ТипУзел Свойство = null;
            if (!(Узел == Неопределено))
            {
        ...
        Массив ПолучитьСвойстваПоСсылке(ТипУзел Узел, object _Путь = null, bool СоздатьСвязь = false)
        {
            ...
            if (ТипЗнч(Путь) == Тип("Строка"))
            {
                ...
                {
                    var оУзел = _оУзел.Значение as ТипУзел;
            ...
            if (СоздатьСвязь == Истина)
            {
                ...
                foreach (КлючИЗначение _Узел in Результаты)
                {
                    ...
                    var сУзел = _Узел.Значение as ТипУзел;
                    var оУзел = сУзел;
        ...
        void ОбъектыОбновитьДобавить(ТипУзел оУзел)
        {
        ...
        public object ОбновитьПредставление(ТипУзел _Узел = null)
        {
            ...
            try
            {
                ...
                else if (ОбъектыОбновить.Количество() != 0)
                {
                    ...
                    {
                        к = к - 1;
                        Узел = ОбъектыОбновить.Получить(0) as ТипУзел;
        ...
        Структура ПолучитьАргументы(ТипУзел Узел)
        {
        ...
        public void ДобавитьСобытие(ТипУзел Узел, string Событие, object Значение = null)
        {
            object зСвойство = null;
            ТипУзел уСвойство = null;
            ...
            {
                var Свойства = ПолучитьСвойстваПоСсылке(зСвойство as ТипУзел);
                foreach (КлючИЗначение _Свойство in Свойства)
                {
                    var Свойство = _Свойство.Значение as ТипУзел;
        ...
        string ОбъектФорма(ТипУзел Узел, dynamic Свойства)
        {
        ...
        public object СоздатьСвойства(ТипУзел Узел, string шСвойства, object _Служебный = null, bool? вКонец = null)
        {
            //var св = Неопределено;
        ...
        object ДобавитьСвойство(ТипУзел УзелСвойства, ТипУзел стрСвойства, bool Служебный = false)
        {
        ...
        void ОбновитьСвязи(ТипУзел Узел)
        {
            ...
            if (!(Связи == Неопределено))
            {
                foreach (КлючИЗначение элУзел in Связи as Соответствие)
                {
                    ...
                    var оУзел = элУзел.Значение as ТипУзел;
                    var Изменения = УзелСвойство(оУзел, "Изменения") as Соответствие;
        ...
        object ПостроитьСтруктуру(ТипУзел Узел)
        {
            var УзелИмя = Узел.Имя;
            if (!(УзелИмя == ""))
            {
        ...
        object ОбработатьОбъект(dynamic Узел, bool Инициализация = false)
        {
            ...
            var Свойства = УзелСвойство(Узел, "Свойства") as ТипУзел;
            if (Свойства == Неопределено)
            {
            ...
            { // инициализация
                ...
                if (Узел == Корень)
                {
                    var уВерсии = УзелСвойство(Свойства.д, "Версии") as ТипУзел;
            ...
            if (_Рефлектор.МетодСуществует(Библиотека, ИмяФункции))
            {
                ...
                {
                    ОбновитьСвязи(св.Ключ as ТипУзел);
                }
                if (Обновить == Истина)
                { // запустить повторно
                    ОбъектыОбновить.Добавить(Узел);
        ...
        object ВызватьФункцию(ТипУзел Узел)
        {
        ...
        object Интерпретировать(ТипУзел Узел, object _ЭтоАтрибут = null, bool НачальныйУзел = true)
        {
            //Перем Имя, Значение, 
            var Результат = Неопределено;
            ...
            while (!(СледУзел == Неопределено))
            {
                ...
                {
                    var Элемент = Интерпретировать(Узел.Дочерний) as ТипУзел;
                    Состояние = НайтиСоседний(Элемент.Атрибут, Значение as string).Значение;
                }
                else if (Имя == "Первый")
                {
                    var Список = Интерпретировать(Узел.Дочерний) as ТипУзел;
        ...
        public ТипУзел НайтиПоКоду(string Код, ТипУзел Старший)
        {
            var Найден = Неопределено;
            ТипУзел Дочерний = null;
            ТипУзел Соседний = null;
            ...
            return Найден as ТипУзел;

        }


        public ТипУзел ПолучитьУзел(object Код, ТипУзел Старший = null)
        {
            var Узел = Узлы.Получить(Код) as ТипУзел;
            if (!(Узел == Неопределено))
            {
                return Узел;
            }
            ...
            foreach (КлючИЗначение _знСтр in мСтр)
            {
                ...
                else
                {
                    if (Узел == Неопределено)
                    {
                        Узел = new ТипУзел("Код, Имя, Значение, Дочерний, Соседний, Атрибут", Код, "");
                    }
                    if (Ключ == "И")
                    {
                        Ключ = "Имя";
        ...
        ТипУзел НовыйУзел(ТипУзел Узел, bool Служебный = false)
        {
            string НовыйКод;
        ...
        public ТипУзел НовыйРодитель(ТипУзел Дочерний, Структура СтруктураУзла, bool Служебный = false)
        {
            ...
            var нУзел = НовыйУзел(СтруктураУзла as ТипУзел, Служебный);
        ...
        public void УдалитьРодителя(ТипУзел Дочерний)
        {
        ...
        ТипУзел НовоеЗначениеУзла(ТипУзел Узел, dynamic Значение = null, bool Служебный = false, object _Добавить = null, bool Обновить = true)
        {
            ...
            var нУзел = НовыйДочерний(Узел, Значение, Служебный, !(ДочернийУзел == Неопределено)) as ТипУзел;
            if (Обновить)
            {
                ОбновитьУзел(Узел);
            }
        ...
        public ТипУзел НовыйДочерний(ТипУзел Старший, Структура СтруктураУзла, bool Служебный = false, bool ВКонец = false)
        {
            ТипУзел зНовыйУзел;
            ...
            { // существующий узел
                зНовыйУзел = СтруктураУзла as ТипУзел;
            }
            else
            {
                зНовыйУзел = НовыйУзел(СтруктураУзла as ТипУзел, Служебный);
            }
        ...
        public ТипУзел НовыйСоседний(ТипУзел Старший, Структура СтруктураУзла, bool Служебный = false)
        {
            СтруктураУзла.Вставить("Старший", Старший);
        ...
        ТипУзел НовыйПоследний(ТипУзел Старший, Структура СтруктураУзла, bool Служебный = false)
        {
            var УзелСоседний = Старший.Соседний;
        ...
        public ТипУзел НовыйАтрибут(ТипУзел Старший, ТипУзел СтруктураУзла, bool Служебный = false)
        {
            СтруктураУзла.Вставить("Старший", Старший);
            СтруктураУзла.Вставить("Родитель", Старший);
        ...
        public void УдалитьУзел(ТипУзел Узел, bool Совсем = true, bool Цепочку = false, bool НачальныйУзел = true)
        {
        ...
        public void КопироватьУзел(ТипУзел Узел, Соответствие Буфер, bool ПервыйВызов = true)
        {
        ...
        ТипУзел НайтиСоседний(ТипУзел Узел, string ИмяУзла)
        {
            while (!(Узел == Неопределено))
            {
                if (Узел.Имя == ИмяУзла)
        ...
        ТипУзел НайтиАтрибут(ТипУзел Узел, string ИмяАтрибута, object ЗначениеАтрибута = null)
        {
            Узел = Узел.Атрибут;
            while (!(Узел == Неопределено))
            {
        ...
        public string СохранитьДанные()
        {
            ...
            {
                if (!(элУзел.Значение == Неопределено && !(Служебный(элУзел.Значение as ТипУзел))))
                {
        ...
        void ПрочитатьВетку(ТипУзел Узел)
        {
            if (!(Узел == Неопределено))
            {
        ...
        public object НайтиУзел(ТипУзел Узел, object ЗначениеУзла, string ИмяУзла = "", bool ПервыйВызов = true)
        {
            ...
            while (НомерЭлемента < СтруктураHTML.Количество() - 1)
            {
                ...
                else if (УзелСтрока == "_at")
                {
                    ...
                    while (!(УзелСтрока == "_rt"))
                    {
                        ...
                        НовыйАтрибут(Узел as ТипУзел, ИмяЗначение(стрУзла.attrName, стрУзла.attrVal));
                        НомерЭлемента = НомерЭлемента + 1;
                        УзелСтрока = СтруктураHTML.Получить(НомерЭлемента) as string;
                    }
                    НомерЭлемента = НомерЭлемента + 1;
                ...
                {
                    НомерЭлемента = НомерЭлемента + 1;
                    var дУзел = НовыйДочерний(Узел as ТипУзел, ИмяЗначение());
        ...
        public pagedata(showdata обПроцесс, string Текст = "", string знБазаДанных = "", string знИмяДанных = "", string знПозицияДанных = "") : base("pagedata")
        {
            ...
            Пустой = НовыйУзел(new ТипУзел("Имя, Родитель", "Пустой", Неопределено), Истина);
