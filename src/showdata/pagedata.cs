...
namespace showdata
{
    class pagedata : functions
    {
        ...
        object СвойстваВСтуктуру(УзелСвойства)
        {
        ...
        object СтруктуруВСтроку(знСтруктура)
        {
        ...
        object КопироватьВетку(Узел, Цель = Неопределено, Старший, Родитель, ЭтоАтрибут = Ложь, ПервыйВызов = Истина, Служебный = Истина, ПараметрыЗамены = Неопределено)
        {
            ...
            КопияУзел = Цель.НовыйУзел(Новый Структура("Имя, Значение, Старший, Родитель", ИмяУзла, ЗначениеУзла, Старший, Родитель), Служебный || Служебный(Узел));
            ...
            if (Узел.Дочерний == Неопределено)
            {
                if (ИмяУзла == "Узел")
                {
                    if (!(ПараметрыЗамены == Неопределено))
                    {
                        ...
                        {
                            КопияУзел.Вставить("Дочерний", Цель.НовыйУзел(Новый Структура("Имя, Значение, Старший, Родитель", "Строка", Параметр, Узел, Узел), Служебный || Служебный(Узел)));
                        }
                    }
                }
            }
