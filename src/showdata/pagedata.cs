...
namespace showdata
{
    class pagedata : functions
    {
        ...
        public class тУзел : Структура
        {
            ...
            public dynamic п { get { return (тУзел)Получить("п"); } set { Вставить("п", value); } }
            public dynamic д { get { return (тУзел)Получить("д"); } set { Вставить("д", value); } }
        ...
        тУзел ИмяЗначение(string Имя = "", string Значение = "")
        {
            return Новый_Структура("Имя, Значение", Имя, Значение) as тУзел;
        }
        ...
        object УзелСвойство(object Узел, string Свойство)
        {
            Перем УзелСвойство = null;
            if (!(Узел == Неопределено))
            {
                (Узел as Структура).Свойство(Свойство, ref УзелСвойство);
            }
            return Вернуть(УзелСвойство);
        } // УзелСвойство(Узел)


        object ЗначениеСвойства(тУзел _знУзелСвойство, string Имя = null)
        {
            var Значение = Неопределено;
            тУзел знУзелСвойство = null;

            if (!(Имя == Неопределено))
            {
                знУзелСвойство = (тУзел)УзелСвойство(_знУзелСвойство, Имя);
            }
        ...
        string ПоказатьУзел(тУзел Узел, string Атрибуты = "", string Дочерний = "", bool ЭтоАтрибут = false)
        {
            var Представление = "";
        ...
        // Создать копию ветки
        public тУзел КопироватьВетку(dynamic Узел, pagedata Цель, тУзел Старший, тУзел Родитель, bool ЭтоАтрибут = false, bool ПервыйВызов = true, bool знСлужебный = true, Структура ПараметрыЗамены = null)
        {
            ...
            тУзел КопияУзел = Цель.НовыйУзел(Новый_Структура("Имя, Значение, Старший, Родитель", ИмяУзла, ЗначениеУзла, Старший, Родитель), знСлужебный || Служебный(Узел));
        ...
        bool СравнитьИмя(тУзел Узел, string Тип,string Имя)
        {
        ...
        object НайтиОбъекты(тУзел Узел, string оПоз, string оТип, string оИмя)
        {
        ...
        тУзел СвойствоОбъекта(тУзел Узел, string ИмяСвойства)
        {
            тУзел Свойство = null;
            if (!(Узел == Неопределено))
            {
        ...
        Массив ПолучитьСвойстваПоСсылке(тУзел Узел, object _Путь = null, bool СоздатьСвязь = false)
        {
            ...
            if (ТипЗнч(Путь) == Тип("Строка"))
            {
                ...
                {
                    var оУзел = _оУзел.Значение as тУзел;
            ...
            if (СоздатьСвязь == Истина)
            {
                ...
                foreach (КлючИЗначение _Узел in Результаты)
                {
                    ...
                    var сУзел = _Узел.Значение as тУзел;
                    var оУзел = сУзел;
        ...
        void ОбъектыОбновитьДобавить(тУзел оУзел)
        {
        ...
        object ОбновитьПредставление(тУзел _Узел = null)
        {
            ...
            try
            {
                ...
                else if (ОбъектыОбновить.Количество() != 0)
                {
                    ...
                    {
                        к = к - 1;
                        Узел = ОбъектыОбновить.Получить(0) as тУзел;
        ...
        Структура ПолучитьАргументы(тУзел Узел)
        {
        ...
        public void ДобавитьСобытие(тУзел Узел, string Событие, object Значение = null)
        {
            object зСвойство = null;
            тУзел уСвойство = null;
            ...
            {
                var Свойства = ПолучитьСвойстваПоСсылке(зСвойство as тУзел);
                foreach (КлючИЗначение _Свойство in Свойства)
                {
                    var Свойство = _Свойство.Значение as тУзел;
        ...
        string ОбъектФорма(тУзел Узел, dynamic Свойства)
        {
        ...
        public object СоздатьСвойства(тУзел Узел, string шСвойства, object _Служебный = null, bool? вКонец = null)
        {
            //var св = Неопределено;
            ...
            { // парсер
                ...
                if (!(стр == ""))
                {
                    ...
                    if (т > к)
                    {
                        св = НовыйДочерний(св, ИмяЗначение(Имя, "" + Зн), Служ, !(вКонец == Ложь && (bool)Служ == Истина));
                        к = т;
                    }
                    else
                    {
                    ...
                    while (п < м.Количество())
                    {
                        ...
                        { // добавить дочерний узел в атрибут
                            ...
                            НовыйДочерний(ат, ИмяЗначение(Имя, "" + Зн), Служ, (bool)Служ == Истина);
                        }
                        п = п + 1;
                    }
                }
        ...
        object ДобавитьСвойство(тУзел УзелСвойства, тУзел стрСвойства, bool Служебный = false)
        {
        ...
        void ОбновитьСвязи(тУзел Узел)
        {
            ...
            if (!(Связи == Неопределено))
            {
                foreach (КлючИЗначение элУзел in Связи as Соответствие)
                {
                    ...
                    var оУзел = элУзел.Значение as тУзел;
                    var Изменения = УзелСвойство(оУзел, "Изменения") as Соответствие;
        ...
        object ПостроитьСтруктуру(тУзел Узел)
        {
            var УзелИмя = Узел.Имя;
            if (!(УзелИмя == ""))
            {
        ...
        object ОбработатьОбъект(dynamic Узел, bool Инициализация = false)
        {
            ...
            var Свойства = УзелСвойство(Узел, "Свойства") as тУзел;
            if (Свойства == Неопределено)
            {
            ...
            { // инициализация
                ...
                var ИмяФункции = Узел.ТипОбъекта + "_Свойства";
                ...
                if (Узел == Корень)
                {
                    var уВерсии = УзелСвойство(Свойства.д, "Версии") as тУзел;
            ...
            var ИмяФункции = Узел.ТипОбъекта + "_Модель";
            if (Рефлектор.МетодСуществует(Библиотека, ИмяФункции))
            {
                Сообщить("м. " + Узел.Код);
                var Параметры = Новый_Массив();
                ...
                var Обновить = Рефлектор.ВызватьМетод(Библиотека, ИмяФункции, Параметры);
                foreach (КлючИЗначение св in Изменения)
                {
                    ОбновитьСвязи(св.Ключ as тУзел);
                }
                if (Обновить == Истина)
                { // запустить повторно
                    ОбъектыОбновить.Добавить(Узел);
        ...
        object ВызватьФункцию(тУзел Узел)
        {
        ...
        object Интерпретировать(тУзел Узел, object _ЭтоАтрибут = null, bool НачальныйУзел = true)
        {
            //Перем Имя, Значение, 
            var Результат = Неопределено;
            ...
            while (!(СледУзел == Неопределено))
            {
                ...
                {
                    var Элемент = Интерпретировать(Узел.Дочерний) as тУзел;
                    Состояние = НайтиСоседний(Элемент.Атрибут, Значение as string).Значение;
                }
                else if (Имя == "Первый")
                {
                    var Список = Интерпретировать(Узел.Дочерний) as тУзел;
        ...
        public тУзел НайтиПоКоду(string Код, тУзел Старший)
        {
            ...
            return Найден as тУзел;

        }


        public тУзел ПолучитьУзел(object Код, тУзел Старший = null)
        {
            var Узел = Узлы.Получить(Код) as тУзел;
            if (!(Узел == Неопределено))
            {
                return Узел;
            }
            ...
            foreach (КлючИЗначение _знСтр in мСтр)
            {
                ...
                else
                {
                    if (Узел == Неопределено)
                    {
                        Узел = Новый_Структура("Код, Имя, Значение, Дочерний, Соседний, Атрибут", Код, "") as тУзел;
                    }
                    if (Ключ == "И")
                    {
                        Ключ = "Имя";
        ...
        тУзел НовыйУзел(Структура _Узел, bool Служебный = false)
        {
            var Узел = _Узел as тУзел;
        ...
        public тУзел НовыйРодитель(тУзел Дочерний, Структура СтруктураУзла, bool Служебный = false)
        {
            ...
            var нУзел = НовыйУзел(СтруктураУзла as тУзел, Служебный);
        ...
        public void УдалитьРодителя(тУзел Дочерний)
        {
        ...
        тУзел НовоеЗначениеУзла(тУзел Узел, dynamic Значение = null, bool Служебный = false, object _Добавить = null, bool Обновить = true)
        {
            ...
            var нУзел = НовыйДочерний(Узел, Значение, Служебный, !(ДочернийУзел == Неопределено)) as тУзел;
            if (Обновить)
            {
                ОбновитьУзел(Узел);
            }
        ...
        public тУзел НовыйДочерний(тУзел Старший, Структура СтруктураУзла, bool Служебный = false, bool ВКонец = false)
        {
            тУзел зНовыйУзел;
            ...
            { // существующий узел
                зНовыйУзел = СтруктураУзла as тУзел;
            }
            else
            {
                зНовыйУзел = НовыйУзел(СтруктураУзла as тУзел, Служебный);
            }
        ...
        public тУзел НовыйСоседний(тУзел Старший, Структура СтруктураУзла, bool Служебный = false)
        {
            СтруктураУзла.Вставить("Старший", Старший);
        ...
        тУзел НовыйПоследний(тУзел Старший, Структура СтруктураУзла, bool Служебный = false)
        {
            var УзелСоседний = Старший.Соседний;
        ...
        public тУзел НовыйАтрибут(тУзел Старший, Структура СтруктураУзла, bool Служебный = false)
        {
            СтруктураУзла.Вставить("Старший", Старший);
            СтруктураУзла.Вставить("Родитель", Старший);
        ...
        public void УдалитьУзел(тУзел Узел, bool Совсем = true, bool Цепочку = false, bool НачальныйУзел = true)
        {
        ...
        public void КопироватьУзел(тУзел Узел, Соответствие Буфер, bool ПервыйВызов = true)
        {
        ...
        тУзел НайтиСоседний(тУзел Узел, string ИмяУзла)
        {
            while (!(Узел == Неопределено))
            {
                if (Узел.Имя == ИмяУзла)
        ...
        тУзел НайтиАтрибут(тУзел Узел, string ИмяАтрибута, object ЗначениеАтрибута = null)
        {
            Узел = Узел.Атрибут;
            while (!(Узел == Неопределено))
            {
        ...
        object СохранитьДанные()
        {
            ...
            {
                if (!(элУзел.Значение == Неопределено && !(Служебный(элУзел.Значение as тУзел))))
                {
        ...
        void ПрочитатьВетку(тУзел Узел)
        {
            if (!(Узел == Неопределено))
            {
        ...
        public object НайтиУзел(тУзел Узел, object ЗначениеУзла, string ИмяУзла = "", bool ПервыйВызов = true)
        {
            ...
            while (НомерЭлемента < СтруктураHTML.Количество() - 1)
            {
                ...
                else if (УзелСтрока == "_at")
                {
                    ...
                    while (!(УзелСтрока == "_rt"))
                    {
                        ...
                        НовыйАтрибут(Узел as тУзел, ИмяЗначение(стрУзла.attrName, стрУзла.attrVal));
                        НомерЭлемента = НомерЭлемента + 1;
                        УзелСтрока = СтруктураHTML.Получить(НомерЭлемента) as string;
                    }
                    НомерЭлемента = НомерЭлемента + 1;
                ...
                {
                    НомерЭлемента = НомерЭлемента + 1;
                    var дУзел = НовыйДочерний(Узел as тУзел, ИмяЗначение());
        ...
        public pagedata(showdata обПроцесс, string Текст = "", string знБазаДанных = "", string знИмяДанных = "", string знПозицияДанных = "") : base("pagedata")
        {
            ...
            Рефлектор = Новый_Рефлектор();
            Пустой = НовыйУзел(Новый_Структура("Имя, Родитель", "Пустой", Неопределено) as тУзел, Истина);
