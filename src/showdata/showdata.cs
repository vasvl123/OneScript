...
namespace onesharp
{
    class showdata : Onesharp
    {
        ...
        {
            var Размер = Цел(Стр.Длина(стр) / 3);
            if (Размер < 3)
            {
                Размер = 3;
            }
        ...
        Вкладка НоваяСтруктураВкладка(object Данные, string ТипВкладки, string Заголовок, string Режим, string УзелКод)
        {
            ...
            if (Данные is pagedata) НоваяВкладка.Данные = Данные;

            if (!(ТипВкладки == "prop"))
            {
                if (!(ТекущаяВкладка is null))
                {
                    мВкладки.Вставить((int)ТекущаяВкладка + 1, НоваяВкладка.ИдВкладки);
                }
                else
                {
        ...
        pagedata ПолучитьДанные(Задача структЗадача)
        {
        ...
        Вкладка НоваяВкладка(Задача структЗадача)
        {
            ...
            else if (Данные is ДвоичныеДанные)
            {
                var Режим = "image";
                var ТипВкладки = "win";
        ...
        object ВыполнитьДействия(Задача структЗадача)
        {
            ...
            { // загрузить данные с дата-сервера
                ...
                { // получен ответ дата-сервера
                    ...
                    {
                        _Данные.Корень.Дочерний.Дочерний.Вставить("Значение", Запрос.с.ИмяДанных);
                    }
                ...
                if (Запрос.Свойство("ЗапросДанных"))
                {
                    if (Запрос.с.ЗапросДанных.с.Команда == "ЗагруженныеДанные")
                    {
                        ...
                        foreach (КлючИЗначение элОбъектДанных in ВсеДанные)
                        {
                            ...
                            var ОбъектДанных = элОбъектДанных.Значение as Структура;
                            Заголовок.Вставить("ИмяДанных", ОбъектДанных.с.ИмяДанных);
                            Заголовок.Вставить("ПозицияДанных", ОбъектДанных.с.ПозицияДанных);
                            Заголовок.Вставить("БазаДанных", ОбъектДанных.с.БазаДанных);
                            Заголовок.Вставить("Размер", ОбъектДанных.с.Количество);
            ...
            if (Действие == Команда["Авторизация"] as string || Действие == Команда["Регистрация"] as string)
            {
                ...
                { // получен ответ дата-сервера
                    ...
                    {
                        var стрУзел = Узел.Новый("Имя, Значение", (string)Параметр.Ключ, (string)Параметр.Значение);
                        кУзел = Данные.НовыйДочерний(Узел, стрУзел, Истина, Истина);
                    }
                    if (ЗапросДанные.с.ПрошелАвторизацию)
                    {
